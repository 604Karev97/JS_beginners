...
const map = {
  ...
  render(snakePointsArray, foodPoint, barrierPoint) {
    ...
    const barrierPointKey = `x${ barrierPoint.x }_y${ barrierPoint.y }`;
    const foodCell = this.cells[foodPointKey];
    const barrierCell = this.cells[barrierPointKey];
...
const game = {
  ...
  reset() {
    ...
    this.barrier.setCoordinates(this.getRandomFreeCoordinate());
    this.render();
  },
  ...
  getRandomFreeCoordinate() {
    const excludeFood = [this.food.getCoordinates(), ...this.snake.getBody()];
    const excludeBarrier = [this.barrier.getCoordinates(), ...this.snake.getBody()];
    ...
    while (true) {
      ...
      if (!excludeFood.some((exPoint) => randomPoint.x === exPoint.x && randomPoint.y === exPoint.y)) {
        return randomPoint;
      }
      if (!excludeBarrier.some((exPoint) => randomPoint.x === exPoint.x && randomPoint.y === exPoint.y)) {
        return randomPoint;
      }
    }
  },
  ...
  finish() {
    ...
    alert(`Вы закончили игру со счетом: ${score} очков(a).`);
  },
  ...
  tickHandler() {
    ...
    if (this.barrier.isOnPoint(this.snake.getNextStepHeadPoint())) return this.finish();
